...
fn build_ui(app: &adw::Application) {
    ...
    // Headless PGO bench: when PLAYER_GTK_BENCH is set, drive the UI through its
    // own hot paths (page switches, search, album-grid + hero rescale, detail
    // pages) and quit — so a `-Cprofile-generate` build run under a headless
    // compositor collects coverage for player-gtk's OWN code (the shared
